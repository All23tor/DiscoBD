...
bool operator<(const CowBlock& lhs, const CowBlock& rhs) {
  return lhs.block_idx < rhs.block_idx;
}
bool operator<(const CowBlock& block, int idx) {
  return block.block_idx < idx;
}
bool operator<(int idx, const CowBlock& block) {
  return idx < block.block_idx;
}
} // namespace

const fs::path disk_path = fs::current_path() / "disk";
const fs::path blocks_path = fs::current_path() / "blocks";

DiskInfo globalDiskInfo = {-1, -1, -1, -1, -1};

Address Address::from_tree(int plate, int surface, int track, int sector) {
  return {plate + globalDiskInfo.plates *
...
const char* load_sector(Address sector_address) {
  ...
  auto it = CowBlock::loaded_blocks.find(block);
  if (it == CowBlock::loaded_blocks.end())
    std::tie(it, std::ignore) = CowBlock::loaded_blocks.insert(CowBlock(block));
  return it->data() + globalDiskInfo.bytes *
...
char* load_writeable_sector(Address sector_address) {
  ...
  if (it == CowBlock::loaded_blocks.end())
    std::tie(it, std::ignore) =
        CowBlock::loaded_blocks.emplace(CowBlock(block));
